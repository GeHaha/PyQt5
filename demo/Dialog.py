# -*- coding: utf-8 -*-
"""
Created on Fri Sep 28 21:50:14 2018
...
class Example(QWidget):
    ...
    def showDialog(self):
        sender = self.sender()
        sex = ['男','女']
        if sender == self.bt1: #按下按钮1，显示输入对话框，第一个字符串是一个对话标题，第二个是对话框中的一个消息。 对话框返回输入的文本和布尔值。 如果我们点击Ok按钮，布尔值为true。
            text,ok = QInputDialog.getText(self,'修改姓名','请输入姓名：')
            if ok:
                self.lb6.setText(text) #如果按下ok，则标签6的text值是从对话框接收的文本
